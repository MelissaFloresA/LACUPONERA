...
class OfertasModel extends Model
{
                    ...
                    <table class="coupon-table">
                        <tr>
                            ...
                            <td class="coupon-details" style="padding-left: 20px; padding-right: 20px;">
                                ...
                                <p style="margin-top: 10px;"><strong>$<?= $cupon['PrecioO'] ?></strong>
                                    <del>$<?= $cupon['PrecioR'] ?></del></p>
    ...
    //FUNCION DE FILTAR CUPONES POR CLIENTE
    public function getCuponesFiltrados($idCliente)
    {
        $query = "SELECT c.*, v.ID_Venta, v.Fecha_Compra, v.Veces_Canje, v.Cantidad, v.Codigo_Cupon 
              FROM cupones c
              INNER JOIN ventas v ON c.ID_Cupon = v.ID_Cupon
              WHERE v.ID_Cliente = :idCliente AND v.Estado = 1";

        $parametros = [':idCliente' => $idCliente];
        $cupones = $this->get_query($query, $parametros);
        ...
        foreach ($cupones as $cupon) {
            if ($cupon['Veces_Canje'] > 0) {
                $resultado['canjeados'][] = $cupon;
            } elseif ($cupon['Fecha_Final'] < $fechaActual) {
                $resultado['vencidos'][] = $cupon;
            } else {
                $resultado['disponibles'][] = $cupon;
            }
        }
